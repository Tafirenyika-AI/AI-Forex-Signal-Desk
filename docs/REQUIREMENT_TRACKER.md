...
## P0-04, P0-05, P1-01 through P1-06, UX-01 through UX-10, research tracks — **NOT STARTED**
...
- `git diff --stat` scoped to: `src/risk/governor.py`, `src/run_loop.py`, `src/broker/alpaca.py`, `src/data/db.py`, `src/dashboard/app.py`, `src/authorization/service.py`, `src/execution/service.py`, one migration script, 6 new test files, this tracker. Nothing in dashboard auth/other unrelated files touched.
- Full test suite: **54/54 passing** (18 original + 36 new across P0-01, P0-02, P0-03), including the dashboard smoke test against the real (migrated) production database.
- No real broker calls in tests, no real orders, no live-account risk-SETTING changes beyond the one additive P0-02 migration — governor/authorization logic verified via fake broker objects, an isolated in-memory SQLite engine, and (for the PaperBroker-specific check) a real `PaperBroker` instance with only its network call monkeypatched. The new kill-switch functions were ALSO live-verified directly against the real production DB using a fake `user_id` that matches no real account, then cleaned up immediately.
- No currently-open trades were touched, inspected for closure, or otherwise acted on — all changes affect *future* risk-governor/authorization decisions on the next live cycle or the next manual approval, not anything already open.

## Continuation state

Natural next slice: **P0-04** (protective-order verification/reconciliation) — a natural partner to both P0-02's kill-switch work and P0-03's "unprotected position -> inf" fail-closed design (a symptom detector, not the actual fix P0-04 asks for: durable follow-up, recovery, honest crypto-target-support disclosure). **P0-05** (auth/session hardening) is a clean, self-contained, well-specified piece of work if a change of pace is preferred over more risk/execution-path changes. Branch `hardening/p0-order-safety` is pushed to GitHub (not merged) for review; the P0-02 migration has ALREADY been applied live (unlike everything else, which is code-only until deployed) — worth flagging to whoever reviews this that the DB schema and the `main` branch's code are now slightly ahead of each other (harmless: the new column/table are additive and unused by any code path not on this branch).
